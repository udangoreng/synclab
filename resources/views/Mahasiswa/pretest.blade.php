...
.pretest-card:hover { transform: translateY(-4px); box-shadow: 0 12px 28px rgba(0,0,0,0.12); }
...
.btn-absen:hover { background: #059669; }
.btn-absen.disabled { background: #9ca3af; cursor: not-allowed; }
...
.upload-info p { margin: 4px 0; font-size: 0.85rem; }
...
.upload-area i { font-size: 48px; color: #3b82f6; margin-bottom: 12px; }
...
@media (max-width: 768px) {
    ...
    .pretest-grid { grid-template-columns: 1fr; }
    ...
    <aside class="sidebar">
        ...
        <div class="sidebar-nav" id="sidebarNav">
            <ul class="nav-menu">
                ...
                <li class="nav-item active"><i class="fas fa-file-alt"></i> Pretest</li>
                <li class="nav-item has-sub">
...
(function() {
    ...
    function renderPretest() {
        ...
        if (filteredData.length === 0) {
            pretestGrid.innerHTML = '<div class="empty-state" style="grid-column:1/-1; text-align:center; padding:40px;">Belum ada pretest yang tersedia</div>';
            return;
        }
    ...
    function submitUpload() {
        ...
        showConfirm('Konfirmasi Upload', `Apakah Anda yakin ingin mengupload laporan "${selectedFile.name}" untuk ${currentUploadItem.modul}?`, () => {
    ...
    function showConfirm(title, message, callback) {
        document.querySelector('#confirmModal .modal-header h3').innerHTML = `<i class="fas fa-question-circle"></i> ${title}`;
