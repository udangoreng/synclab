...
(function() {
    ...
    const demoCredentials = {
        student: {
            ...
            role: 'student',
            redirect: 'dashboard-mahasiswa.html'
        },
        lecturer: {
    ...
    function handleLogin(e) {
        ...
        if (email === demoCredentials.student.email && password === demoCredentials.student.password) {
            user = demoCredentials.student;
            role = 'student';
            redirectUrl = demoCredentials.student.redirect;
        } else if (email === demoCredentials.lecturer.email && password === demoCredentials.lecturer.password) {
            user = demoCredentials.lecturer;
            role = 'lecturer';
            redirectUrl = demoCredentials.lecturer.redirect;
        } else if (email === demoCredentials.admin.email && password === demoCredentials.admin.password) {
            user = demoCredentials.admin;
            role = 'admin';
            redirectUrl = demoCredentials.admin.redirect;
        } else {
            // Demo fallback - untuk testing, semua email dengan format apapun bisa login
            // asalkan passwordnya benar (minimal 6 karakter)
            // Ini hanya untuk demo, di production harus pakai backend
            if (password.length >= 6) {
                // Tentukan role berdasarkan email
                if (email.includes('dosen') || email.includes('lecturer')) {
                    role = 'lecturer';
                    redirectUrl = 'dashboard-dosen.html';
                } else if (email.includes('admin')) {
                    role = 'admin';
                    redirectUrl = 'dashboard-admin.html';
                } else {
                    role = 'student';
                    redirectUrl = 'dashboard-mahasiswa.html';
                }
                user = { email, role, redirect: redirectUrl };
            } else {
                const passwordGroup = passwordInput.closest('.input-group');
                showError(passwordGroup, 'Email atau password salah');
                return;
            }
        }
    ...
    // Add floating label effect for pre-filled inputs
